...
def etl():
    ...
    while fn.end_date <= current_date.date():
        ...
        if len(THREADS) >= 5 and start_time_local - time.time() < 1:
            ...
            start_time_local = time.time()
        if count == 15:
            file.save_json(EARNINGS_DATA, str(fn.start_date))
            current_date = datetime.now()
            EARNINGS_DATA = list()
            count = 0
    ...
    for t in THREADS:
        t.join()
    file.save_json(EARNINGS_DATA, str(current_date.date()))
...
def profile():
    ...
    _df = financial.Process('profile').remove_names(symbol, ['symbol'])
    start_time_local = time.time()
    row = None
...
def historical_price_full(asc: bool = True):
    ...
    _df = financial.Process('historical_price_full').remove_names(symbol, ['date', 'symbol'], 1, asc)
    start_time_local = time.time()
    row = None
    for index, row in _df.iterrows():
